...
def harness(monkeypatch):
    settings = SimpleNamespace(studio_key='presenter-test-key', studio_mysql_password='local-test-password')
    ...
    return SimpleNamespace(client=TestClient(app, raise_server_exceptions=False), settings=settings,
                           store=store, redis=redis, context=context,
                           headers={'X-Studio-Key': 'presenter-test-key'})


@pytest.fixture
def offer():
    return {'package_id': 'VT-001', 'name': 'Synthetic Maui Escape', 'destination': 'Maui',
            'total_price': '5890.00', 'available_rooms': 4, 'eligible_reward_base': '4712.00',
            'cancellation': 'Fictional refundable hotel', 'departure_date': '2027-04-10',
...
@pytest.mark.parametrize('field,value', [
    ...
    ('package_id', 'VT-' + 'A' * 14),
    ('total_price', '-0.01'), ('total_price', '1.001'),
    ('eligible_reward_base', '-1'), ('eligible_reward_base', '0.001'),
    ('available_rooms', -1), ('available_rooms', 1.5),
    ('name', '  '), ('departure_date', 'not-a-date'),
])
def test_invalid_offer_never_reaches_mysql(harness, offer, field, value):
    offer[field] = value
    response = harness.client.post('/api/studio/offers', headers=harness.headers, json=offer)
...
def test_path_ids_rejected_before_delete_or_context_access(harness, package_id):
    deleted = harness.client.request('DELETE', '/api/studio/offers/' + package_id,
                                    headers=harness.headers, json={'expected_updated_at': '2026-09-17 12:00:00.000000'})
    fetched = harness.client.get('/api/studio/context/' + package_id, headers=harness.headers)
...
def test_optimistic_conflict_is_not_reported_as_success(harness, offer):
    harness.store.mutate.side_effect = HTTPException(409, 'This row changed since you loaded it')
    response = harness.client.put('/api/studio/offers/VT-001', headers=harness.headers,
...
def test_successful_insert_only_mutates_mysql(harness, offer):
    ...
    assert harness.store.mutate.call_args.kwargs['offer'].total_price == Decimal('5890.00')
    assert not harness.redis.mock_calls
    harness.context.call.assert_not_awaited()
...
def test_restore_requires_exact_confirmation(harness):
    for confirmation in ['', 'restore', 'RESTORE ALL']:
        response = harness.client.post('/api/studio/restore', headers=harness.headers, json={'confirm': confirmation})
        assert response.status_code == 422
    harness.store.mutate.assert_not_called()
    response = harness.client.post('/api/studio/restore', headers=harness.headers, json={'confirm': 'RESTORE'})
...
@pytest.mark.parametrize('result', [{'error': 'service unavailable'}, {'ok': False}, {}, {'package_id': 'VT-999'}, {'found': False}])
...
def test_store_conflicting_version_locks_row_and_never_commits(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        store.mutate('delete', package_id='VT-001', expected='2026-09-17 12:00:00.000000')
    assert exc.value.status_code == 409
    cursor.execute.assert_called_once_with('SELECT updated_at FROM offers WHERE package_id=%s FOR UPDATE', ('VT-001',))
