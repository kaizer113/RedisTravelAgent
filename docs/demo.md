...
Optional live-data segment, approximately two minutes. The visual option is [Data Studio](data-studio.md): open it, edit VT-001’s price to 5790, save, and watch its Redis copy update. Use the Context Retriever check, then restore the original price. The same sequence is also available in a terminal at this repository:
